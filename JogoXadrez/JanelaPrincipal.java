...
public class JanelaPrincipal extends JFrame {
    ...
    public void reagir(CasaGUI casaClicada) {
        if (primeiroClique) {
            if (casaClicada.possuiPeca()) {
				int c = casaClicada.getCorPeca();
				if(((c == 0) && (cor.equals("branca"))) || ((c == 1) && (cor.equals("preta")))){
					casaClicadaOrigem = casaClicada;
					casaClicadaOrigem.destacar();
					primeiroClique = false;
				}
				else{
					JOptionPane.showMessageDialog(this, "Peca do oponente.");
				}
            }
            else {
            ...
            primeiroClique = true;
            client.enviaCasa(casaClicadaOrigem.getPosicaoX(), casaClicadaOrigem.getPosicaoY(),
                    casaClicadaDestino.getPosicaoX(), casaClicadaDestino.getPosicaoY());
            
            if(jogo.podePromover(casaClicadaOrigem.getPosicaoY()))
            {
                while(!fecharJanela)
                {
                    String[] opcoes = {"Rainha", "Cavalo", "Torre", "Bispo"};
                    int nome = JOptionPane.showOptionDialog(null, "Escolha uma das peças para promover o peão", 
                    "Clique em um botão", JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE, null, opcoes, opcoes[0]);
                    
                    if((nome == 0) || (nome == 1) || (nome == 2) || (nome == 3))
                    {
                        jogo.promover(nome);
						client.enviaPromocao(nome);
						fecharJanela = true;
                    }
				}
			}
            ...
            if(jogo.empate && !jogo.afogado)
            {
                JOptionPane.showMessageDialog(this, "Jogo empatou, reinicie o jogo se quiser jogar novamente.");
            }
            
            if(jogo.empate && jogo.afogado)
            {
                JOptionPane.showMessageDialog(this, "Empate por afogamento, reinicie o jogo se quiser jogar novamente.");
            }
    ...
    /**
     * Cria um novo jogo e Atualizador o tabuleiro gr�fico.
     */
    ...
    private void initComponents() {
		...
		tabuleiroGUI = new TabuleiroGUI(this, this.cor);
	...
	private void initComponentsInv() {
